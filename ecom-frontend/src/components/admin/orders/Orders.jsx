...
const Orders = () => {
  ...
  // Check if there are NO orders at all (totalElements === 0)
  const hasNoOrdersAtAll = !pagination || pagination?.totalElements === 0;
  // Check if current page is empty but there are orders in other pages
  const currentPageEmpty = (!adminOrder || adminOrder?.length === 0) && !hasNoOrdersAtAll;
  ...
  return (
    <div className='pb-6 pt-20'>
        {hasNoOrdersAtAll ? (
            ...
            <div className='flex flex-col items-center justify-center text-gray-600 py-10'>
                <FaShoppingCart size={50} className='mb-3'/>
                <h2 className='text-2xl font-semibold'>No Orders on This Page</h2>
                <p className='text-gray-500 mt-2'>Try navigating to a different page</p>
                <OrderTable adminOrder={[]} pagination={pagination}/>
            </div>
        ) : (
