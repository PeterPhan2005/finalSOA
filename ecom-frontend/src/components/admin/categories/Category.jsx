...
const Category = () => {
  ...
  // Check if there are NO categories at all in the system
  const hasNoCategoriesAtAll = pagination?.totalElements === 0;
  
  // Check if current page is empty but categories exist on other pages
  const currentPageEmpty = (!categories || categories?.length === 0) && !hasNoCategoriesAtAll;
  ...
  return (
    <div>
      ...
      {!hasNoCategoriesAtAll && !currentPageEmpty && (
            ...
            <>
              {currentPageEmpty && (
                <div className="flex flex-col items-center justify-center text-gray-500 py-6 mb-4 bg-gray-50 rounded-lg border border-gray-200">
                  <FaFolderOpen size={40} className="mb-2 text-gray-400" />
                  <h3 className="text-xl font-medium">No categories on this page</h3>
